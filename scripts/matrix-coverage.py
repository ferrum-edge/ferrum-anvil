...
def pinned_release():
    """The lab's default gateway release (`release` line of RELEASE.lock)."""
    for line in open(os.path.join(ROOT, "lab/gateway/RELEASE.lock")):
        parts = line.split()
...
def run_release(r):
    try:
        return json.load(open(os.path.join(r, "summary.json"))).get("gateway_release")
    except (OSError, ValueError):
        return None


def latest_lab_results(release):
    """base id -> {runs, variants: {scenario id: {profile, passes}}} from the
    newest run per profile against `release` (runs against another supported
    release are ignored). Scenario variants (`UP-004.expired`,
    ...
    for r in runs:
        ...
        if "-" not in name or run_release(r) != release:
            continue
...
def main():
    ...
    refs = scan(cases)
    release = os.environ.get("ANVIL_LAB_RELEASE") or pinned_release()
    lab = latest_lab_results(release)
    rows = [classify(c, refs[c["id"]], lab, overrides) for c in cases]
    os.makedirs(OUT_DIR, exist_ok=True)
    totals = {}
    for r in rows:
        totals[r["status"]] = totals.get(r["status"], 0) + 1
    json.dump({"source": os.path.relpath(MATRIX, ROOT), "gateway_release": release, "total": len(rows), "totals": totals, "cases": rows},
    ...
    md = ["# Failure-matrix coverage", "",
          ...
          f"against Ferrum Edge {release} (the lab's default pin; `ANVIL_LAB_RELEASE` selects another release).",
