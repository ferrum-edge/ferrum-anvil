...
| Webview → Rust backend | A compromised renderer | Narrow typed commands; lock enforced in backend; secrets returned only as references; file access only through grants: the backend shows the native dialog, keeps the chosen path and returns an opaque, purpose-bound token that file commands require (no command takes a path; grants expire, are revoked on lock, and are refused if the file or a folder on its path was replaced); capability allowlist (`capabilities/default.json`, no save dialog, no filesystem plugin) |
