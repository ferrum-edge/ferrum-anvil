...
| `upstream.route_request_timeout.not_dispatched` | same bytes | same, plus `:49211`, `:38314` | Same deadline expired before any backend got the request (upload, hooks, DNS, admission) or in retry backoff. `error_class` `dispatch_policy_rejected`. **Before dispatch this is a 504 `backend_timeout` that never reached a backend** (ferrum-edge/ferrum-edge#5762); in retry backoff an earlier attempt did reach one. |
