...
- **JWT-SVID auth** (`AuthConfig::JwtSvid`): `Authorization: Bearer <JWT-SVID>` (header and prefix configurable). Sources: `FetchJWTSVID` for the configured audiences (and optionally one SPIFFE ID), a variable or vault value, or a file read at send time. At least one audience is required.
