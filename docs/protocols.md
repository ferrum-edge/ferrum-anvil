...
|---|---|---|---|---|
| WebSocket, HTTP/1.1 Upgrade (`ws://`, `wss://`) | **Yes.** Send text, binary (hex) and ping. Close with a code and reason. Cancel. The transcript streams live. | **Yes.** Scripted messages, then a stop on `expect_messages`, idle close, peer close or the total deadline. | **Yes — `websocket_session`.** One session per unit (own connection): opened / handshake rejected / not opened, closed cleanly, messages sent and received, close codes by who closed; round-trip times only when `expect_messages` pairs the script with replies. Lab: `LOAD-013-ws`. | Phases DNS, connect, proxy, TLS, handshake, session. 101 validation (`Upgrade`, `Connection`, `Sec-WebSocket-Accept`). Subprotocol negotiation. Close code, reason and `closed_by`. 1006 is reported as abnormal. 1009 is split into local limit vs peer limit. |
| WebSocket over HTTP/2 extended CONNECT (RFC 8441) | **Yes**, same commands | **Yes** | **Yes — `websocket_session`** (a fresh connection per session) | A separate bootstrap. The client waits for the peer's `SETTINGS_ENABLE_CONNECT_PROTOCOL`. A 200 is success. Works over TLS (ALPN `h2`) and h2c. |
...
4. **gRPC.** No outbound compression, and no retry or service-config semantics. Proto imports resolve by attachment file name only. A manually sent call opens its own connection (including a fresh QUIC connection over HTTP/3); only the load engine's persistent mode reuses pooled channels. gRPC-Web is limited by the protocol to unary and server streaming, cannot use server reflection, sends no request trailer frame, and supports protobuf messages only.
...
7. **Connections.** Session adapters open a fresh connection per execution or session. Unlike HTTP/1.1, HTTP/2 and HTTP/3, they do not use the pool — except gRPC calls in a load run's persistent mode, which reuse a per-virtual-user channel.
