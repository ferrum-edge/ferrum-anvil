...
- **Combinations refused before traffic** (`unsupported_combination`, `phase = prepare`, `dispatch = not_dispatched`):
  - a proxy with UDP/DTLS, except `udp://` through an HBONE proxy (§3.9): HTTP CONNECT and SOCKS5 tunnels carry only TCP; an HBONE proxy carries UDP as a datagram tunnel, but not DTLS or a PROXY protocol envelope; and no proxy carries the QUIC connection of UDP through a MASQUE proxy;
...
- **Refused before traffic.** DTLS (`dtls://`) through an HBONE proxy (Anvil's DTLS adapter reads and writes its own UDP socket and does not yet run over a datagram channel); a PROXY protocol envelope through an HBONE proxy (mesh relays never read it); UDP through a MASQUE proxy together with an HBONE proxy; an auth profile (payloads are verbatim, as for direct UDP).
...
8. **HBONE.** One tunnel per inner connection or UDP session (no HTTP/2 stream sharing across executions). No double-HBONE (tunnel inside a tunnel), no HBONE over HTTP/3 (QUIC), and no Anvil-side capture: Anvil is a direct client of the endpoint. A `CONNECT` refusal is reported with its public status and body only; which mesh policy refused is never claimed. UDP through HBONE uses the Ferrum Mesh datagram framing (`[u16 length][payload]` records behind the `udp` marker), which is Ferrum's, not an Istio standard; DTLS through an HBONE tunnel is not implemented yet (the DTLS adapter owns its UDP socket; a follow-up once it can run over a datagram channel); ICMP errors toward the destination are seen by the endpoint only, and the reason an endpoint ends a tunnel is never sent, so it is never claimed.
