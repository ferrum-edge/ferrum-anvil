...
pub struct WebSocketConfig {
    ...
    /// Negotiated RFC 7692 `permessage-deflate` parameters. `None` (the
    /// default) means no per-message compression: RSV1 must stay clear.
    /// Read when the WebSocket is created; changing it later has no effect.
    #[cfg(feature = "deflate")]
...
impl WebSocketConfig {
    ...
    /// Set [`Self::deflate`].
    #[cfg(feature = "deflate")]
    pub fn deflate(mut self, deflate: Option<DeflateConfig>) -> Self {
...
pub struct WebSocketContext {
    ...
    /// The negotiated `permessage-deflate` codec, if any.
    #[cfg(feature = "deflate")]
    deflate: Option<deflate::DeflateCodec>,
}
...
impl WebSocketContext {
    ...
    fn _new(role: Role, mut frame: FrameCodec, config: WebSocketConfig) -> Self {
        ...
        Self {
            ...
            #[cfg(feature = "deflate")]
            deflate: config.deflate.map(deflate::DeflateCodec::new),
            config,
        }
    }
        ...
        Stream: Read + Write,
    {
        ...
        let frame = match message {
            #[cfg(feature = "deflate")]
            Message::Text(data) => self.data_frame(data.into(), OpData::Text)?,
            #[cfg(feature = "deflate")]
            Message::Binary(data) => self.data_frame(data, OpData::Binary)?,
    ...
    /// A single-frame data message; compressed (RSV1 set) when
    /// `permessage-deflate` is negotiated and compresses outgoing messages.
    #[cfg(feature = "deflate")]
    fn data_frame(&mut self, data: bytes::Bytes, opcode: OpData) -> Result<Frame> {
        if let Some(compressed) = self.deflate.as_mut().map(|d| d.compress(&data)).transpose()?.flatten() {
            let mut frame = Frame::message(compressed, OpCode::Data(opcode), true);
            frame.header_mut().rsv1 = true;
            return Ok(frame);
        }
        Ok(Frame::message(data, OpCode::Data(opcode), true))
    }
    ...
    fn read_message_frame(&mut self, stream: &mut impl Read) -> Result<Option<Message>> {
        ...
        // Connection_.
        {
            ...
            // RFC 7692 §6: RSV1 marks a compressed message, and only on the
            // first frame of a data message.
            #[cfg(feature = "deflate")]
            let starts_message = matches!(hdr.opcode, OpCode::Data(OpData::Text | OpData::Binary));
            #[cfg(feature = "deflate")]
            if hdr.rsv1 && !hdr.rsv2 && !hdr.rsv3 && starts_message && self.deflate.is_none() {
                return Err(Error::Protocol(ProtocolError::CompressedMessageNotNegotiated));
            }
            #[cfg(feature = "deflate")]
            let rsv1_defined = starts_message && self.deflate.is_some();
            #[cfg(not(feature = "deflate"))]
            let rsv1_defined = false;
            if (hdr.rsv1 && !rsv1_defined) || hdr.rsv2 || hdr.rsv3 {
                ...
                let payload = match (data, self.incomplete.as_mut()) {
                    ...
                    (OpData::Continue, Some(incomplete)) => {
                        ...
                        let payload = match self.deflate.as_mut() {
                            Some(d) if d.receiving() => d.decompress(
                                frame.payload(),
                                fin,
                                incomplete.len(),
                                self.config.max_message_size,
                            )?,
                            _ => frame.into_payload(),
                        };
                        #[cfg(not(feature = "deflate"))]
                    ...
                    #[cfg(feature = "deflate")]
                    (OpData::Text | OpData::Binary, _) if compressed => {
                        let d = self.deflate.as_mut().expect("RSV1 was checked against the negotiated codec");
                        d.begin_message();
                        let payload = d.decompress(frame.payload(), fin, 0, self.config.max_message_size)?;
                        let t = if data == OpData::Text { MessageType::Text } else { MessageType::Binary };
                        Ok(Some((payload, t)))
                    }
