...
pub enum CapacityError {
    ...
    /// A `permessage-deflate` message is bigger than the maximum allowed size
    /// once decompressed. Decompression stops as soon as the limit is passed.
    #[cfg(feature = "deflate")]
    #[error("Decompressed message too long: {compressed_size} compressed bytes expand beyond {max_size}")]
    DecompressedMessageTooLong {
        /// Compressed payload bytes of the message received so far.
        compressed_size: usize,
        /// The maximum allowed (decompressed) message size.
        max_size: usize,
    },
}
...
pub enum ProtocolError {
    ...
    /// A data message arrived compressed (RSV1 set on its first frame), but no
    /// per-message compression extension was negotiated.
    #[cfg(feature = "deflate")]
    #[error("Compressed message received, but no permessage-deflate was negotiated")]
    CompressedMessageNotNegotiated,
    /// A `permessage-deflate` message could not be decompressed.
    #[cfg(feature = "deflate")]
    #[error("Invalid compressed message: {0}")]
    InvalidCompressedMessage(String),
}
