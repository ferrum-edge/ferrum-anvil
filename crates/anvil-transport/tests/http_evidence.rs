...
fn plan(url: &str, tls: Option<TlsSettings>) -> HttpPlan {
    ...
    HttpPlan {
        ...
        tls: if https {
            Some(Arc::new(
                tls::prepare(&tls.unwrap_or_else(trust_lab)).expect("tls profile"),
            ))
        } else {
            None
        },
...
fn failure(o: &AttemptOutput) -> &TransportFailure {
    o.observation.failure.as_ref().unwrap_or_else(|| {
        panic!(
            "expected failure, got status {:?}",
            o.observation.response_status
        )
    })
}
...
async fn positive_control_http1_evidence_and_keepalive_reuse_proto_001() {
    ...
    let a = run(&t, &p).await;
    assert!(
        a.observation.failure.is_none(),
        "{:?}",
        a.observation.failure
    );
    ...
    assert_eq!(
        phase_status(&a, Phase::Connect),
        Some(PhaseStatus::Completed)
    );
    assert_eq!(
        phase_status(&a, Phase::Dns),
        Some(PhaseStatus::NotApplicable),
        "IP literal must not show a DNS measurement"
    );
    assert_eq!(
        a.response.as_ref().unwrap().body.completeness,
...
async fn tls_001_untrusted_root_is_confirmed_by_verifier() {
    ...
    assert!(matches!(
        ...
        TlsVerification::Failed {
            problem: FailureKind::TlsUntrustedIssuer,
            ..
        }
    ));
    assert!(
        !tls.peer_certificates.is_empty(),
        "presented chain is evidence even on failure"
    );
}
...
async fn tls_002_hostname_mismatch() {
    ...
    let a = run(
        &HttpTransport::new(),
        &plan(&fx.url_host("localhost", "/"), None),
    )
    .await;
...
async fn tls_003_expired_and_tls_004_not_yet_valid() {
    ...
    let b = run(
        &HttpTransport::new(),
        &plan(&fx2.url_host("localhost", "/"), None),
    )
    .await;
...
async fn tls_005_missing_client_certificate_records_request_evidence() {
    ...
    let fx = tls_fixture(
        &pki().server,
        ClientAuth::Required {
            ca_pem: pki().client_ca.cert.clone(),
        },
    )
    .await;
    let a = run(
    ...
    assert!(
        matches!(
            f.kind,
            FailureKind::TlsAlertAfterHandshake | FailureKind::TlsAlertReceived
        ),
...
async fn tls_006_rejected_client_certificate_and_tls_008_valid_mtls() {
    ...
    let a = run(
        &HttpTransport::new(),
        &plan(&fx.url_host("localhost", "/"), Some(bad)),
    )
    .await;
    let f = failure(&a);
    assert!(
    ...
    let b = run(
        &HttpTransport::new(),
        &plan(&fx.url_host("localhost", "/echo"), Some(good)),
    )
    .await;
    assert!(
        b.observation.failure.is_none(),
        "{:?}",
    ...
    assert!(
        ...
        "private key material must never appear in evidence"
    );
}
...
async fn tls_009_https_to_plaintext_port_is_protocol_mismatch() {
    ...
    let a = run(
        &HttpTransport::new(),
        &plan(&format!("https://localhost:{}/", fx.addr.port()), None),
    )
    .await;
    let f = failure(&a);
...
async fn tls_010_handshake_stall_hits_tls_deadline() {
    init();
    let fx = raw::serve("127.0.0.1:0", RawMode::AcceptStall, None)
        .await
        .unwrap();
    let a = run(
        &HttpTransport::new(),
    ...
    assert_eq!(
        phase_status(&a, Phase::TlsHandshake),
        Some(PhaseStatus::TimedOut)
    );
    assert_eq!(
        phase_status(&a, Phase::Connect),
        Some(PhaseStatus::Completed)
...
async fn tls_011_reset_during_handshake_is_not_attributed_to_certs() {
    init();
    let fx = raw::serve("127.0.0.1:0", RawMode::AcceptReset, None)
        .await
        .unwrap();
    let a = run(
        &HttpTransport::new(),
    ...
    let f = failure(&a);
    assert!(
        matches!(f.kind, FailureKind::TlsReset | FailureKind::TlsPeerClosed),
        "{:?} {}",
        f.kind,
        f.message
    );
    ...
    assert_eq!(
        *v,
        TlsVerification::Bypassed {
            would_have_failed: Some(FailureKind::TlsUntrustedIssuer)
        }
    );
...
async fn tls_014_mtls_pool_isolation_between_profiles() {
    ...
    let cns: Vec<Option<String>> = fx
        ...
        .filter_map(|e| match e.event {
            anvil_fixtures::GroundTruth::TlsHandshakeCompleted { client_cert_cn, .. } => {
                Some(client_cert_cn)
            }
            _ => None,
        })
...
async fn up_011_body_idle_stall_after_200_is_incomplete_not_success() {
    ...
    let a = run(
        &HttpTransport::new(),
        &plan(&fx.url("/stall-body/5000"), None),
    )
    .await;
    assert_eq!(a.observation.response_status, Some(200));
    let f = failure(&a);
...
async fn up_012_reset_mid_body() {
    init();
    let fx = raw::serve("127.0.0.1:0", RawMode::ResetMidBody { sent: 2048 }, None)
        .await
        .unwrap();
    let a = run(&HttpTransport::new(), &plan(&fx.url(false, "/"), None)).await;
    assert_eq!(a.observation.response_status, Some(200));
    assert!(
        matches!(
            failure(&a).kind,
            FailureKind::BodyReset | FailureKind::BodyIncomplete
        ),
        "{:?}",
        failure(&a)
    );
...
async fn reset_after_request_may_have_been_sent() {
    init();
    let fx = raw::serve("127.0.0.1:0", RawMode::ResetAfterRequest, None)
        .await
        .unwrap();
    ...
    assert!(
        matches!(
            f.kind,
            FailureKind::ResetBeforeResponse | FailureKind::ClosedBeforeResponse
        ),
        "{:?}",
        f
    );
    assert_eq!(
        a.observation.dispatch,
        DispatchState::MayHaveBeenSent,
        "bytes left the client; processing is possible"
    );
    assert!(
        fx.log.count_requests() >= 1,
        "ground truth: the fixture did read the request"
    );
}
...
async fn headers_timeout_distinct_from_body_timeout() {
    init();
    let fx = raw::serve("127.0.0.1:0", RawMode::ReadThenStall, None)
        .await
        .unwrap();
    let a = run(&HttpTransport::new(), &plan(&fx.url(false, "/"), None)).await;
...
async fn proto_005_h2c_against_http1_only_peer_fails_cleanly() {
    ...
    let f = failure(&a);
    assert!(
        a.observation.connection.as_ref().unwrap().tls.is_none(),
        "no TLS handshake occurred for h2c"
    );
    assert!(
        matches!(
            f.kind,
...
async fn local_011_cancel_during_upload_is_canceled_with_dispatch_uncertainty() {
    ...
    let outs = t
        .execute(&p, 0, AttemptReason::Initial, &EventCtx::none(), &cancel)
        .await;
    let a = outs.last().unwrap();
    assert_eq!(
        a.observation.failure.as_ref().unwrap().kind,
...
async fn dns_override_skips_resolution() {
    ...
    let fx = fxhttp::serve("127.0.0.1:0", None).await.unwrap();
    let mut p = plan(
        &format!("http://api.anvil.test:{}/status/200", fx.addr.port()),
        None,
    );
    p.dns.overrides = vec![anvil_domain::settings::DnsOverride {
    ...
    assert!(
        hdrs.iter()
            .any(|(n, v)| n == "host" && v.starts_with("api.anvil.test")),
        "Host header keeps the requested authority"
    );
}
