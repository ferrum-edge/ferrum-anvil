        ...
        let (mut conn, reused) = match pooled {
            ...
            None => {
                ...
                let connecting = async {
                    match resumption {
                        Some(r) => connector::establish_resumable(&mut rec, &target, &plan.dns, &plan.timeouts, r).await,
                        None => (connector::establish(&mut rec, &target, &plan.dns, &plan.timeouts, plan.proxy.as_ref()).await, None),
                    }
                };
