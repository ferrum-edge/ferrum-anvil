...
fn prepared_content_type(spec: &RequestSpec) -> Option<String> {
    ...
    prepare_http(spec, &resolver, &attachments, &EffectiveSettings::default(), false, &["https"])
        .unwrap()
        .content_type
}
...
fn curl_soap_content_types_reach_preparation() {
    ...
    assert_eq!(
        prepared_content_type(&quoted_action).as_deref(),
        Some("application/soap+xml; charset=utf-8; action=\"urn:lookup\"")
    );

    let no_action = imported_request(
        b"curl -H 'Content-Type: application/soap+xml; charset=utf-8' -d '<E/>' https://example.test",
    );
    assert_eq!(prepared_content_type(&no_action).as_deref(), Some("application/soap+xml; charset=utf-8"));

    let bare = imported_request(
        b"curl -H 'Content-Type: application/soap+xml' -d '<E/>' https://example.test",
    );
    assert_eq!(prepared_content_type(&bare).as_deref(), Some("application/soap+xml"));

    let soap_11 = imported_request(
        b"curl -H 'Content-Type: text/xml' -H 'SOAPAction: \"urn:lookup\"' -d '<E/>' https://example.test",
    );
